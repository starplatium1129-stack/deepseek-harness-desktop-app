...
桌面管理与工作空间是两个独立 WebContentsView。0.4.3 起，先同步切换原生可见性、页面状态与焦点，再执行目标视图的可选入场动画。禁止在显示目标前等待隐藏渲染器的截图、解码或动画；正常启动的后台限流会让这些等待停住。装饰效果失败也不能阻塞导航。
...
玻璃材质与背景独立。应用的两种预设共享各页面原有的默认底色；参考页的彩色网格仅用于观察折射，不是开启玻璃后的默认背景。

布局容器如果承载 fixed 弹窗，不能直接施加 backdrop-filter：它会改变固定定位的包含块。0.4.1 将侧栏材质移到独立的绝对定位装饰子层，内容保持正常层叠和视口定位；装饰层不接收指针事件，也不参与无障碍树。复用时应同样检查弹窗的祖先滤镜、transform 与 contain，不能只验证玻璃表面本身。
...
- 本地壁纸仅经受信主进程的文件选择器导入；上游页面不获得文件系统或通用桌面管理接口。专用颜色桥只允许当前回环主框架上报 light / dark / system 三种偏好。
- 颜色偏好、文字颜色和材质必须同源更新，不能只从桌面向上游单向设置。内置主题服务的变更事件会反向同步保存状态与桌面控件，来源标记和状态版本防止回环及过期回写。
