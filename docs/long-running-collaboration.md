...
必须使用 `--shared` 共享服务。原来的八个单任务工具继续保留；0.3.0 总共提供十四个工具，0.4.0 增加三个[原派发会话回传](codex-dispatcher-return.md)工具。单个普通 submit_task 不会自动变成项目循环。用户要求当前 Codex 主代理审核时，使用绑定 dispatcher 的单任务流程；本后台循环只在明确选择独立协调器时使用。
