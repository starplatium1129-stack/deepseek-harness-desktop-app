...
- **外观预设**：经典与玻璃材质，浅色、深色及跟随系统；支持本地壁纸、渐变、纯色、模糊与遮罩。设置本地保存，切换不重启服务。
- **连续动效**：可打断的弹簧动画、触发位置展开的弹窗、侧栏反向和跨页面衔接；玻璃边缘对实际背景进行屏幕空间折射。参考苹果公开设计原则独立实现，不是 Apple 原生组件的逐像素复刻。
- **Windows 桌面习惯**：Ctrl+1～4 切页，F6 交接导航与内容焦点，原生编辑菜单；F1 或顶部 `?` 查看快捷键。支持 75%～200% 同步显示缩放、窄窗口布局、窗口边界与最大化状态记忆。
