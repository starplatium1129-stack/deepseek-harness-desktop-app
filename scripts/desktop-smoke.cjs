...
async function main() {
  ...
  const packaged = process.argv.includes('--packaged') || !!process.env.DSH_SMOKE_EXECUTABLE;
  ...
  const packagedCandidates = [process.env.DSH_SMOKE_EXECUTABLE, path.join(root, 'release/win-unpacked/DeepSeek Harness Desktop.exe'), path.join(root, `release/appearance-${version}/win-unpacked/DeepSeek Harness Desktop.exe`)].filter(Boolean);
  const executablePath = packaged ? packagedCandidates.find(candidate => fsSync.existsSync(candidate)) || packagedCandidates[0] : require('electron');
  const launch = () => electron.launch({ executablePath, args: packaged ? [] : [root], env, timeout: 60000 });
  ...
  try {
    let page = await app.firstWindow(); trackErrors(page);
    ...
    // Rejected input remains editable; scoped errors survive timers, navigation and state refreshes.
    const keyInput = page.locator('#api-key'), keyMessage = page.locator('#key-message');
    await keyInput.fill('x'.repeat(4097)); await page.keyboard.press('Enter');
    await expect(keyMessage).toBeVisible();
    await expect(keyMessage).toContainText('不能超过 4096 个字符');
    await expect(keyInput).toHaveAttribute('aria-invalid', 'true'); await expect(keyInput).toHaveValue('x'.repeat(4097));
    await page.waitForTimeout(16250); await expect(keyMessage).toBeVisible(); await expect(keyMessage).toContainText('不能超过 4096 个字符');
    await press('Control+4'); await press('Control+3'); await expect(keyMessage).toBeVisible(); await expect(keyMessage).toContainText('不能超过 4096 个字符');
    await keyInput.fill('corrected-placeholder'); await expect(keyInput).not.toHaveAttribute('aria-invalid', 'true');
    await keyInput.fill(''); await page.keyboard.press('Enter'); await expect(keyMessage).toContainText('请输入 API Key');
    await expect(keyInput).toBeFocused(); await expect(keyInput).toHaveAttribute('aria-invalid', 'true');
    await keyInput.fill('corrected-placeholder'); await keyInput.fill('');
    await page.locator('#appearance').click();
    ...
    const workspace = app.context().pages().find(p => p.url().startsWith('http://127.0.0.1:')); trackErrors(workspace);
    ...
    await app.close(); app = await launch(); page = await app.firstWindow(); trackErrors(page);
